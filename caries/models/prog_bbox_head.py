...
class ProgressionBBoxHead(Shared2FCBBoxHead):
    ...
    def loss_and_target(self,
        ...
        cls_reg_targets = self.get_targets(
            sampling_results, batch_gt_instances, rcnn_train_cfg, concat=concat)
        losses = self.loss(
            cls_score,
            bbox_pred,
            rois,
    ...
    def get_targets(self,
        ...
        pos_gt_scores_list = [
            insts.scores[res.pos_assigned_gt_inds]
            for insts, res in zip(batch_gt_instances, sampling_results)
        ]
        labels, label_scores, label_weights, bbox_targets, bbox_weights = multi_apply(
    ...
    def _get_targets_single(self, pos_priors: Tensor, neg_priors: Tensor,
        ...
        labels = pos_priors.new_full((num_samples, ),
                                     self.num_classes,
                                     dtype=torch.long)
        label_scores = pos_priors.new_zeros((num_samples, self.num_classes + 1), dtype=torch.float32)
        label_scores[:, -1] = 1
        ...
        if num_pos > 0:
            ...
            if label_scores.shape[1] == 4:
                label_scores[:num_pos, 0] = torch.clip((0.5 - pos_gt_scores) / 0.5, 0, 1)
                label_scores[:num_pos, 1] = 1 - torch.abs(0.5 - pos_gt_scores) / 0.5
                label_scores[:num_pos, 2] = torch.clip((pos_gt_scores - 0.5) / 0.5, 0, 1)
                label_scores[:num_pos, 3] = 0
            elif label_scores.shape[1] == 3:
                label_scores[:num_pos, 0] = 1 - pos_gt_scores
                label_scores[:num_pos, 1] = pos_gt_scores
                label_scores[:num_pos, 2] = 0
    ...
    def _predict_by_feat_single(
        ...
        if rcnn_test_cfg is None:
            ...
            pos_scores = scores[:, :-1].clone()
            pos_scores /= pos_scores.sum(1, keepdim=True)
            if pos_scores.shape[1] == 3:
                progs = 0.5 * pos_scores[:, 1] + pos_scores[:, 2]
            else:
                progs = pos_scores[:, 1]

            probs = scores.clone()
            if True:
                scores = torch.column_stack((1 - scores[:, -1:], scores[:, -1:]))
                bboxes = bboxes[:, :4]

            det_bboxes, det_labels, indices = multiclass_nms(
                bboxes,
                scores,
                ...
                return_inds=True,
                box_dim=box_dim)
            box_indices = indices // (scores.shape[1] - 1)
            results.bboxes = det_bboxes[:, :-1]
            results.scores = det_bboxes[:, -1]
            results.progs = progs[box_indices]
            results.labels = det_labels
            results.probs = probs[box_indices]
